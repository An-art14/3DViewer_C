...
void MainWindow::loadSettings() {
    ...
    vertexColor = settings.value("vertexColor", QColor(Qt::black)).value<QColor>();
...
{
    if(model.num_vertices != 0 && currentValues.scale != arg1&& std::abs(arg1) > std::numeric_limits<double>::epsilon()) {
...
void MainWindow::on_ModelColorButton_clicked()
{
    QColor color = QColorDialog::getColor(modelColor, this, "Choose model color");
        if (color.isValid()) {
            modelColor = color;
            //setColor(false, modelColor.redF(), modelColor.greenF(), modelColor.blueF());
            ui->openGLWidget->setModelColor(modelColor);
            transformAndDraw();
        }
}
...
void MainWindow::on_VertexColorButton_clicked() {
    QColor color = QColorDialog::getColor(vertexColor, this, "Choose vertex color");
    if (color.isValid()) {
        vertexColor = color;
        ui->openGLWidget->setVertexColor(vertexColor);
        transformAndDraw();
